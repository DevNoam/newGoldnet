...
REM Warn if TGMS is not under default tgms path
...
REM Check for businessnet username dont allow installation if provided account is different
...
REM reg query "%registryPath%" >nul 2>nul
if %errorlevel% neq 0 (
    echo BusinessNet is installed incorrectly/ Not installed.
    pause
    exit
)
